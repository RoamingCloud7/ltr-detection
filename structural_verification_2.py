...
Candidate = namedtuple(
    "Candidate",
    "seq_id ltr1_start ltr1_end ltr2_start ltr2_end identity score gap"
)
...
def compute_features(seq, five_start, five_end, three_start, identity, trna_tails):
    # 1) LTR similarity → 0–40
    sim_score = max(0.0, min(identity,1.0)) * 40.0
    ...
    total = sim_score + pbs_score + ppt_score + domain_score + canon_bonus
...
def analyse_candidate(rec, cand, trna_tails):
    # compute features in both orientations
    featA = compute_features(rec.seq, cand.ltr1_start, cand.ltr1_end,
                             cand.ltr2_start, cand.identity, trna_tails)
    featB = compute_features(rec.seq, cand.ltr2_start, cand.ltr2_end,
                             cand.ltr1_start, cand.identity, trna_tails)
    chosen = featB if featB['total_score'] > featA['total_score'] else featA

    # report LTR similarity as percent 0–100
    return {
        'seq_id':        cand.seq_id,
...
def main():
    ...
    # load tRNAs, index fasta, load candidates
    ...
    for cand in cands:
        ...
        if cand.seq_id not in idx:
            continue
        rec = idx[cand.seq_id]
        results.append(analyse_candidate(rec, cand, trna_tails))
    ...
    fields = [
        ...
        'PPT_hit','PPT_ratio','domains','total_score'
    ]
    ...
    for r in results:
        if float(r['total_score']) < thr:
            break
        rec = idx[r['seq_id']]
        s1,e1 = r['ltr1_start'], r['ltr1_end']
        s2,e2 = r['ltr2_start'], r['ltr2_end']
